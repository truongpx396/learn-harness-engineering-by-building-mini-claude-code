...
def task_create(subject: str, description: str = "", blocked_by: list | None = None) -> str:
    TASKS_DIR.mkdir(exist_ok=True)
    existing = [int(p.stem.split("_")[1]) for p in TASKS_DIR.glob("task_*.json")]
    task_id = max(existing, default=0) + 1
    task = {"id": task_id, "subject": subject, "description": description,
            "status": "pending", "owner": "", "blockedBy": blocked_by or []}
            ...
            for _ in range(50):
                ...
                for tool_call in msg_obj.tool_calls:
                    if tool_call.function.name == "idle":
                        ...
                        output = self._exec(name, tool_call.function.name, json.loads(tool_call.function.arguments))
...
TOOL_HANDLERS = {
    ...
    "task_create":       lambda **kw: task_create(kw["subject"], kw.get("description", ""), kw.get("blocked_by")),
}
...
TOOLS = [
    ...
    {"type": "function", "function": {"name": "claim_task", "description": "Claim a task from the board by ID.",
     "parameters": {"type": "object", "properties": {"task_id": {"type": "integer"}}, "required": ["task_id"]}}},
    {"type": "function", "function": {"name": "task_create", "description": "Create a new task on the task board.",
     "parameters": {"type": "object", "properties": {
         "subject": {"type": "string"},
